...
class TestWebServerEndpoints:
    ...
    def test_model_set_persists_excitech_ai_chat_routing(self, monkeypatch):
        monkeypatch.setattr(
            "hermes_cli.model_cost_guard.expensive_model_warning",
            lambda *_args, **_kwargs: None,
        )
        ...
        resp = self.client.post(
            ...
            json={
                ...
                "model": "general-main",
                "excitech_gateway_domain": "excitech",
                "excitech_gateway_agent": "developer",
            },
        )
        assert resp.status_code == 200
        ...
        assert model_cfg["provider"] == "excitech-gateway"
        assert model_cfg["excitech_gateway_domain"] == "excitech"
        assert model_cfg["excitech_gateway_agent"] == "developer"
        assert "excitech_gateway_mode" not in model_cfg

        info = self.client.get("/api/model/info")
        assert info.status_code == 200
        assert info.json()["excitech_gateway_domain"] == "excitech"
        assert info.json()["excitech_gateway_agent"] == "developer"
