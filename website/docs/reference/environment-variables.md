...
| `HERMES_IGNORE_RULES` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, and memory entries. Chat still preloads mandatory product-knowledge skills. Equivalent to `--ignore-rules`. |
| `HERMES_SAFE_MODE` | Troubleshooting mode: disable most customizations — skips plugin discovery and MCP server loading. Chat still preloads mandatory product-knowledge skills. Set automatically by `--safe-mode` (which also sets the two flags above). |
