...
def cmd_chat(args):
    ...
    if not getattr(args, "safe_mode", False) and not getattr(args, "ignore_rules", False):
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    """Offer to configure dashboard auth when the selected bind requires it.
    ...
    when:
      * the bind is loopback and ``--require-auth`` was not requested, or
    ...
    try:
        ...
        if (
            not getattr(args, "require_auth", False)
            and not should_require_auth(host)
        ):
    ...
    print()
    print(
        f"⚠ The dashboard authentication gate is enabled on {host} and "
...
def cmd_dashboard(args):
    ...
    start_server(
        ...
        force_auth=getattr(args, "require_auth", False),
...
def _default_product_knowledge_skills() -> list[str]:
    """Return bundled product-knowledge packs that should preload in chat."""
    try:
    ...
    return [
        ...
        if skill_md.parent.name.strip()
    ]


def _merge_chat_skills(skills: object) -> list[str]:
    """Merge explicit chat skills with bundled product-knowledge packs."""
    merged: list[str] = []
    seen: set[str] = set()

    def _append(items: object) -> None:
        if not items:
            return
        values = [items] if isinstance(items, str) else list(items) if isinstance(items, (list, tuple, set)) else [str(items)]
        for value in values:
    ...
    _append(_default_product_knowledge_skills())
    _append(skills)
    return merged
