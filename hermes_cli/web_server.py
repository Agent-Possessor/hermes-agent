...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        if provider.strip().lower() == "excitech-gateway":
            # Remove the retired transport selector whenever this config is
            # touched: Excitech model traffic now has one path only.
            model_cfg.pop("excitech_gateway_mode", None)
            model_cfg["excitech_gateway_domain"] = (
                excitech_gateway_domain or "general"
            )
            model_cfg["excitech_gateway_agent"] = (
                excitech_gateway_agent or "assistant"
            )
        else:
            model_cfg.pop("excitech_gateway_domain", None)
            model_cfg.pop("excitech_gateway_agent", None)
            model_cfg.pop("excitech_gateway_provider_policy", None)
            model_cfg.pop("excitech_gateway_mode", None)
        cfg["model"] = model_cfg
